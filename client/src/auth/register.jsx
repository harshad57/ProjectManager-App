                    ...
                    <form method="post" onSubmit={send} className="rounded-lg border">
                        <div className="flex flex-col gap-4 p-6 md:p-8 backdrop-blur-md">
                            <div>
                                ...
                                <input name="name" className="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring" type="text" value={name} onChange={(e) => setname(e.target.value)} required />
                            </div>
                            ...
                            <div>
                                ...
                                <input name="password" className="w-full rounded border bg-gray-50 px-3 py-2 text-gray-800 outline-none ring-indigo-300 transition duration-100 focus:ring" type="password" value={password} onChange={(e) => setpassword(e.target.value)} required />
                            </div>

                            <button className="block rounded-lg bg-gray-800 px-8 text-center text-sm font-semibold text-white outline-none ring-gray-300 transition duration-100 hover:bg-gray-700 focus-visible:ring active:bg-gray-600 md:text-base cursor-pointer h-12 mt-4" type="submit" disabled={loading}>{loading ? <SmallLoaderPage /> : "Register"}</button>
                        </div>

                        <div className="flex items-center justify-center bg-gray-100 p-4">
                            <div className="text-center text-sm text-gray-500">Already have an account ? <div onClick={() => navigate('/login')} className="text-indigo-500 transition duration-100 hover:text-indigo-600 active:text-indigo-700 cursor-pointer">Login</div></div>
                        </div>
                    </form>
                </div>
            </div>
